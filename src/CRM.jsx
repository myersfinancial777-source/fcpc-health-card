...
// =====================================================
// MODULE-LEVEL CACHES — survive component unmounts
// =====================================================
// These persist as long as the page is open, so when the user navigates
// away from the Clients tab and back, data shows up instantly while
// fresh data quietly refreshes in the background.
let _clientsCache = null;                  // full client list
const _propertiesCache = new Map();        // clientId -> properties[]
const _inspectionsCache = new Map();       // propertyId -> inspections[]

// Cached load wrappers — return cached data immediately when available,
// always fire a background fetch to keep cache fresh.
...
async function loadProperties(clientId) {
  ...
  _propertiesCache.set(clientId, data);
  return data;
}
async function loadInspectionsForProperty(propId) {
  const data = await _loadInspectionsForProperty(propId);
  _inspectionsCache.set(propId, data);
  return data;
}

// Mutating wrappers — invalidate cache so next read pulls fresh data.
...
async function deleteClient(id) {
  ...
  _clientsCache = null;
  _propertiesCache.delete(id);
  return result;
}
async function saveProperty(property) {
  const result = await _saveProperty(property);
  _propertiesCache.delete(property.client_id);
  return result;
}
async function deleteProperty(propId) {
  const result = await _deleteProperty(propId);
  // We don't know which clientId owned this property here, so clear all
  // property caches. Cheap because clients re-fetch is only triggered on visit.
  _propertiesCache.clear();
  _inspectionsCache.delete(propId);
  return result;
}

// -- Client list -------------------------------------
export function ClientList({ onSelectClient, onNewClient }) {
  // Initialize from cache so the list shows instantly on tab return.
  const [clients, setClients] = useState(() => _clientsCache || []);
  const [search, setSearch] = useState('');
  const [loading, setLoading] = useState(() => _clientsCache === null);

  useEffect(() => {
    // Always refresh in the background so data stays current.
  ...
  return (
    <div>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 12 }}>
        <h2 style={{ margin: 0, fontSize: 18, color: NAVY, ...F, fontWeight: 700 }}>Clients</h2>
...
export function ClientForm({ clientId, onSave, onCancel }) {
  ...
  useEffect(() => {
    if (clientId) {
      ...
      if (_clientsCache) {
        const found = _clientsCache.find(c => c.id === clientId);
...
export function ClientDetail({ clientId, onBack, onStartInspection, onOpenInspection }) {
  // Hydrate state from cache for instant display on tab return.
  const [client, setClient] = useState(() => {
    if (_clientsCache) return _clientsCache.find(c => c.id === clientId) || null;
    return null;
  });
  const [properties, setProperties] = useState(() => _propertiesCache.get(clientId) || []);
  const [editing, setEditing] = useState(false);
  const [addingProp, setAddingProp] = useState(false);
  const [editingPropId, setEditingPropId] = useState(null);
  const [expandedProp, setExpandedProp] = useState(null);
  // Hydrate inspections state from module cache so prior expansions feel instant.
  const [propInspections, setPropInspections] = useState(() => {
    const initial = {};
    for (const [propId, insps] of _inspectionsCache.entries()) {
      initial[propId] = insps;
    }
    return initial;
  });
  ...
  async function refresh() {
    // Background refresh — if no cache, this is also the first load.
  ...
  async function togglePropExpand(propId) {
    ...
    // Show cached inspections instantly if we have them.
    if (_inspectionsCache.has(propId)) {
      setPropInspections(prev => ({ ...prev, [propId]: _inspectionsCache.get(propId) }));
    }

    // Always fetch fresh in the background to keep data current.
...
export function PropertyForm({ clientId, propertyId, onSave, onCancel }) {
  ...
  useEffect(() => {
    if (propertyId) {
      // Try cache first
      const cached = _propertiesCache.get(clientId);
      if (cached) {
