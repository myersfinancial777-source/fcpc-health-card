...
function createBlank() {
  ...
  return { id: genId(), propertyAddress: '', unitSuite: '', ownerManager: '', planTier: '', date: todayStr(), statuses, photos, itemNotes, notes: '', overallRating: '', createdAt: new Date().toISOString() };
}
...
function PhotoRow({ photos, onAdd, onRemove, onTap }) {
  ...
  return (<div style={{ display: 'flex', gap: 6, flexWrap: 'wrap', marginTop: 8, alignItems: 'center' }}>
    ...
    <button onClick={() => camRef.current?.click()} style={{ width: 56, height: 56, borderRadius: 8, border: `2px dashed ${TEAL}`, background: TEAL_LIGHT, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', cursor: 'pointer', flexShrink: 0 }}>
      <span style={{ fontSize: 18, lineHeight: 1 }}>📸</span><span style={{ fontSize: 7, color: TEAL, fontWeight: 700, ...F, marginTop: 1 }}>Camera</span></button>
    <button onClick={() => fileRef.current?.click()} style={{ width: 56, height: 56, borderRadius: 8, border: `2px dashed ${BORDER_GRAY}`, background: '#fff', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', cursor: 'pointer', flexShrink: 0 }}>
      <span style={{ fontSize: 18, lineHeight: 1 }}>🖼️</span><span style={{ fontSize: 7, color: MED_GRAY, fontWeight: 700, ...F, marginTop: 1 }}>Gallery</span></button>
...
function PdfPreview({ inspection, onClose }) {
  ...
  return (<div id="pdf-preview-root">
    ...
    <div className="no-print" style={{ position: 'sticky', top: 0, zIndex: 200, background: NAVY, padding: '10px 16px', display: 'flex', justifyContent: 'space-between', alignItems: 'center', borderBottom: `3px solid ${TEAL}` }}>
      ...
      <button onClick={() => window.print()} style={{ background: TEAL, color: '#fff', border: 'none', borderRadius: 20, padding: '8px 18px', fontSize: 13, fontWeight: 700, cursor: 'pointer', ...F }}>🖨 Print / Save</button>
    </div>
    <div style={{ background: '#fff', maxWidth: 800, margin: '0 auto', boxShadow: '0 2px 20px rgba(0,0,0,.1)' }}>
      ...
      {SECTIONS.map(sec => (<div key={sec.title} className="no-break">
        ...
        {sec.items.map((item, idx) => { const s = inspection.statuses[item], sc = sC(s), sl = sL(s), hasP = (inspection.photos?.[item] || []).length > 0, note = inspection.itemNotes?.[item] || ''; return (<div key={item}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '5px 28px', fontSize: 11, borderBottom: `1px solid ${BORDER_GRAY}15`, background: idx % 2 === 0 ? '#fff' : LIGHT_GRAY }}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>{hasP && <span style={{ fontSize: 8, color: TEAL, fontWeight: 600 }}>📷</span>}<div style={{ fontWeight: 700, fontSize: 10, padding: '2px 10px', borderRadius: 10, background: sc + '15', color: sc, border: `1px solid ${sc}33`, minWidth: 70, textAlign: 'center' }}>{sl}</div></div></div>
      ...
      {photoEntries.length > 0 && <><div className="page-break" /><div style={{ background: NAVY, color: '#fff', padding: '10px 28px', fontSize: 13, fontWeight: 700, borderLeft: `4px solid ${TEAL}`, marginTop: 8 }}>📷 Photo Documentation</div>
...
export default function App() {
  ...
  // Auto-save current inspection on changes (debounced)
  ...
  function startNew() { const f = createBlank(); setInspections(p => [f, ...p]); saveInspection(f); setCurrentId(f.id); setView('form'); setExpanded({}); }
  ...
  if (view === 'list') return (
    <div style={S.app}>
      ...
      <div style={S.body}>
        ...
        {inspections.length === 0 && <div style={S.empty}><div style={{ fontSize: 40, marginBottom: 12 }}>📋</div><div style={{ fontSize: 15, fontWeight: 600, color: NAVY, ...F }}>No inspections yet</div><div style={{ fontSize: 13, color: MED_GRAY, marginTop: 4, ...F }}>Tap "+ New" to start</div></div>}
          ...
          <div key={insp.id} style={S.card} onClick={() => openI(insp.id)}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
              <div style={{ flex: 1 }}><div style={{ fontSize: 14, fontWeight: 700, color: NAVY, ...F }}>{insp.propertyAddress || 'Untitled Property'}</div>
                ...
                <div style={{ fontSize: 11, color: MED_GRAY, marginTop: 2, ...F }}>{insp.date} • {insp.planTier || 'No plan'}{pc > 0 ? ` • 📷 ${pc}` : ''}</div></div>
            ...
            <button onClick={e => { e.stopPropagation(); if (confirm('Delete?')) delI(insp.id); }} style={S.delBtn}>Delete</button></div>); })}</div></div>);
    ...
    return (<div style={S.app}>
      ...
      <div style={S.body}>
        ...
        {notedItems.length > 0 && <div style={{ background: '#fff', border: `1px solid ${BORDER_GRAY}`, borderRadius: 12, padding: 14, marginBottom: 16 }}>
          <div style={{ fontSize: 12, fontWeight: 700, color: NAVY, marginBottom: 8, ...F }}>📝 Item Notes</div>
        ...
        {pe.length > 0 && <div style={{ background: '#fff', border: `1px solid ${BORDER_GRAY}`, borderRadius: 12, padding: 14, marginBottom: 16 }}>
          <div style={{ fontSize: 12, fontWeight: 700, color: NAVY, marginBottom: 10 }}>📷 Photos ({pc})</div>
        ...
        <button onClick={() => setView('pdf')} style={{ ...S.exportBtn, width: '100%', marginBottom: 10 }}>📄 Preview & Print PDF</button>
            ...
            {sec.items.map((item, idx) => { const ip = cur.photos?.[item] || []; const st = cur.statuses[item]; const showNote = st && st !== 'good'; const noteVal = cur.itemNotes?.[item] || ''; return (
              <div key={item} style={{ padding: '12px 14px', background: idx % 2 === 0 ? '#fff' : LIGHT_GRAY, borderBottom: idx < sec.items.length - 1 ? `1px solid ${BORDER_GRAY}44` : 'none' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 8 }}>
                  ...
                  {ip.length > 0 && <span style={{ fontSize: 10, color: TEAL, fontWeight: 600, ...F }}>📷 {ip.length}</span>}</div>
