...
export default function App() {
  ...
  const [tab, setTab] = useState('inspections'); // inspections | clients
  ...
  // CRM state
  const [crmView, setCrmView] = useState('list'); // list | add | detail
  ...
  function TabBar() {
    return (
      <div style={{ display: 'flex', background: '#fff', borderBottom: `1px solid ${BORDER_GRAY}`, position: 'sticky', top: 0, zIndex: 99 }}>
        {[{ key: 'inspections', label: '📋 Inspections' }, { key: 'clients', label: '👤 Clients' }].map(t => (
  ...
  if (view === 'list') return (
    <div style={S.app}>
      ...
      <div style={S.body}>
        ...
        {inspections.length === 0 && <div style={S.empty}><div style={{ fontSize: 40, marginBottom: 12 }}>📋</div><div style={{ fontSize: 15, fontWeight: 600, color: NAVY, ...F }}>No inspections yet</div><div style={{ fontSize: 13, color: MED_GRAY, marginTop: 4, ...F }}>Tap "+ New" or start one from a client's property</div></div>}
